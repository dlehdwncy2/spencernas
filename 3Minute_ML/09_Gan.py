...
# 생성기 신경망에 사용하는 변수들입니다. #노이즈를 통한 가짜 이미지 생성
...
loss_val_D, loss_val_G = 0, 0 #loss_G, loss_D 결괏값을 받을 변수 설정

for epoch in range(total_epoch):
    for i in range(total_batch):
        batch_xs, batch_ys = mnist.train.next_batch(batch_size) #batch_size 학습시킬 이미지 갯수
                                                                                                    # X: 이미지 numpy 배열화, Y: 이미지 라벨
        noise = get_noise(batch_size, n_noise) #배치 크기 만큼 노이즈를 생성한다.
                                                                        #batchsize : 행 , n_noise : 열

        # 판별기와 생성기 신경망을 각각 학습시킵니다.
        _, loss_val_D = sess.run([train_D, loss_D], #판별기
                                 feed_dict={X: batch_xs, Z: noise})
        _, loss_val_G = sess.run([train_G, loss_G], #신경망
                                 feed_dict={Z: noise})
