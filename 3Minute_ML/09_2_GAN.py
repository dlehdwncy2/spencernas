...
def get_noise(batch_size, n_noise):
    return np.random.uniform(-1., 1., size=[batch_size, n_noise]) #uniform : 균등 분포 -1과 1사이에 랜덤값 추출

# 생성 모델과 판별 모델에 Y 즉, labels 정보를 추가하여
# labels 정보에 해당하는 이미지를 생성할 수 있도록 유도합니다.
G = generator(Z, Y)
D_real = discriminator(X, Y)
D_gene = discriminator(G, Y, True) #가짜 이미지 구분자를 만들 때는 진짜 이미지 구분자에서 사용한 변수를 재사용하기 위해서 True로 설정
...
for epoch in range(total_epoch):
    for i in range(total_batch):
        ...
        _, loss_val_G = sess.run([train_G, loss_G],
                                 feed_dict={Y: batch_ys, Z: noise}) #노이즈와 실제 이미지에 각각에 해당하는 숫자를 힌트로 넣어주는 용도
    ...
    if epoch == 0 or (epoch + 1) % 10 == 0:
        ...
        samples = sess.run(G,
                           feed_dict={Y: mnist.test.labels[:sample_size], #실제 Y와 noise를 넣어볼 시
                                      Z: noise}) #noise 값에 label 추가
        #samples : 노이즈를 통해 얻은 무작위 이미지 [새로운 손글씨]
