...
with tf.Session() as sess:
 ...
 ###################
  ...
  for i in range(10):
      ...
      label=sess.run(y_pred,  feed_dict={x: x_test[test_index].reshape(-1, 32,32, 3), keep_prob: 1.0 })
      label_list.append(label)
      target_list.append(np.argmax(label,1))
...
for i in range(10): #첫 번째부터 열 번째까지의 이미지
    ...
    subplot.set_title('%d' % np.argmax(label_list[i])) #해당 배열에서 가장 높은 값을 가지 인덱스를 예측한 숫자로 출력
